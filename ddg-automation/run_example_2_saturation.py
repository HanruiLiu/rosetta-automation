...
rosetta_scripts_path = os.path.expanduser("../../bqa/rosetta_bin_linux_2018.33.60351_bundle/main/source/bin/rosetta_scripts.mpi.linuxgccrelease")
...
path_to_script = '{{PATH_TO_RESIDUE_DDG}}/ddG-backrub.xml'
residue_to_mutate = ('{{CHAIN_TO_MOVE}}', {{RESIDUE}}, '') # Residue position to perfrom saturation mutatagenesis. Format: (Chain, PDB residue number, insertion code).

def run_flex_ddg_saturation( name, input_path, input_pdb_path, chains_to_move, mut_aa, nstruct_i ):
    output_directory = os.path.join( '{{PATH_TO_RESIDUE_DDG}}/output_{{RESIDUE}}', os.path.join( '%s_%s' % (name, mut_aa), '%02d' % nstruct_i ) )
    ...
    flex_ddg_args = [
        '/lustre/home/users/bqa/rosetta_bin_linux_2018.33.60351_bundle/main/source/bin/rosetta_scripts.mpi.linuxgccrelease',
...
if __name__ == '__main__':
    ...
    for nstruct_i in range(1, nstruct + 1 ):
        for case_name in os.listdir('{{PATH_TO_RESIDUE_DDG}}/inputs'):
            case_path = os.path.join( '{{PATH_TO_RESIDUE_DDG}}/inputs', case_name )
